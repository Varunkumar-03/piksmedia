...
export default function SocialCards({ cards }: SocialCardsProps) {
  ...
  return (
    <div className="w-full py-32 flex flex-col items-center justify-center bg-[#FDFBF7] overflow-hidden">
      <div className="mb-2 text-center">
        <h3 className="text-xl md:text-3xl font-medium tracking-tight text-stone-900">
          Discover our <span className="italic font-serif text-stone-500">premium</span> frames
        </h3>
      </div>
      <section className="flex flex-col items-center w-full py-4 lg:py-8 px-4 md:px-8 relative z-20">
        <div className="flex items-center justify-center w-full max-w-[90rem]">
          <div ref={containerRef} className="fan-layout flex relative justify-center items-center w-full max-w-[80rem] h-[450px] md:h-[550px]">
            {cards.map((card, index) => {
              const image = (
                <div className="relative w-full h-[320px] md:h-[420px] overflow-hidden rounded-[1.5rem] border-2 border-white shadow-2xl bg-stone-100">
                  <img src={card.imgUrl} loading="lazy" alt={card.alt || `Card ${index}`} className="absolute inset-0 w-full h-full object-cover z-10" />
                </div>
              );
              return card.linkUrl ? (
                <a key={index} href={card.linkUrl} target={card.linkUrl.startsWith("http") ? "_blank" : "_self"} rel="noopener noreferrer" className="fan-card absolute block cursor-pointer w-[220px] md:w-[300px]">{image}</a>
              ) : (
                <div key={index} className="fan-card absolute w-[220px] md:w-[300px]">{image}</div>
              );
            })}
          </div>
        </div>
